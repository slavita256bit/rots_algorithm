...
void export_csv(Cubeset ci, Cubeset** data, const char* test_path, int iteration, time_t* now, const char* prefix, const char* name, const char* left_name) {
    ...
    strcat(filename + strlen(test_path), prefix);
    sprintf(filename + strlen(test_path) + strlen(prefix), "%02d", iteration);
    char *filename_time_ptr = get_time_formated(filename + strlen(test_path) + strlen(prefix) + 2, now);
